...
namespace TCPChatServer_SendFile
{
        ...
        */
        public void ClientRecv(string username, TcpClient tcpClient)
        {
            ...
            try
            {
                while (!stopChatServer)
                {
                    ...
                    while (thisUserNetworkStream.DataAvailable)
                    {
                        int bytesRead = thisUserNetworkStream.Read(forwardBuffer, 0, forwardBuffer.Length);

                        // Tách phần header và dữ liệu
                        string headerAndMessage = Encoding.UTF8.GetString(forwardBuffer.Take(bytesRead).ToArray());
                        string[] arrPayload = headerAndMessage.Split(new[] { ';' }, 3);
                        ...
                        if (arrPayload.Length >= 3)
                        {
                            ...
                            else if (msgType == MessageType.FilePart || msgType == MessageType.FileEof)
                            {
                                ...
                                if (dict.TryGetValue(friendUsername, out TcpClient friendTcpClient))
                                {
                                    ...
                                    // Forward file chunk
                                    friendNetworkStream.Write(forwardBuffer, 0, bytesRead);
                                }
                            }
                        }
                    }
        ...
        private void btnListen_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                stopChatServer = true;
                tcpListener.Stop();
                listenThread?.Join();  // Chờ thread hoàn tất
                listenThread = null;
